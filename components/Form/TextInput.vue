...
const inputRoundedStyle = computed(() => slots.prefix ? 'rounded-r' : 'rounded');
...
<template>
    <div :class="`flex realtive overflow-hidden ${wrapperSizeStyles}`">
        <div
            v-if="slots.prefix"
            :class="`px-4 py-2 ${defaultStyle} ${selectedSize} bg-gray-100 dark:bg-slate-800 text-gray-500`"
        >
            <slot name="prefix" />
        </div>
        <input 
            v-model="modelValue"
            type="text"
            :class="`flex-1 block w-full ${inputRoundedStyle} ${defaultStyle} ${selectedSize}`"
